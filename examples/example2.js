var askQuestions = require('./../dist').default
...
const questions = [
  {
    identifier: 'language',
    prompt: `${chalk.green('?')} What language? (ruby, node)`,
    validation: (answer) => {
  ...
  {
    // identifier: 'who',
    prompt: `${chalk.green('?')} What version of node?`,
  ...
  {
    // identifier: 'who',
    prompt: `${chalk.green('?')} What version of ruby?`,
  ...
  {
    // identifier: 'who',
    prompt: `${chalk.green('?')} Project name?`,
    // onDone: (answer) => {
    //   return 'I am no one :-)'
    // }
  },
